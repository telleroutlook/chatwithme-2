...
import { ChatCircleDotsIcon, PlusIcon, TrashIcon, WrenchIcon, FolderOpenIcon, GearSixIcon, XIcon } from "@phosphor-icons/react";
...
export function WorkspaceSidebar({
  ...
  const sections: Array<{
    ...
    { id: "tools", label: t("sidebar_domain_tools"), icon: <WrenchIcon size={14} />, count: toolsCount },
    { id: "resources", label: t("sidebar_domain_resources"), icon: <FolderOpenIcon size={14} />, count: resourcesCount },
    ...
    >
      <div className="space-y-3 border-b border-kumo-line/80 bg-kumo-base/60 p-3">
        <div className="flex items-center justify-between">
          <Text size="xs" variant="secondary">{t("sidebar_workspace")}</Text>
          {mobile && (
            <button
              type="button"
              onClick={onClose}
          ...
          {sections.map((item) => (
            ...
            >
              ...
              {typeof item.count === "number" && item.count > 0 ? (
                <span className="rounded-full bg-kumo-control px-1.5 py-0.5 text-[10px]">{item.count}</span>
              ) : null}
            </button>
          ))}
        </div>
      ...
      {section === "chats" ? (
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <span className="block truncate">
                      <Text size="sm" bold={currentSessionId === session.id}>{session.title}</Text>
                    </span>
                    <span className="mt-0.5 block truncate">
                      <Text size="xs" variant="secondary">{session.lastMessage || t("session_no_messages")}</Text>
                    </span>
                    <div className="mt-1 flex items-center gap-2">
                      <Text size="xs" variant="secondary">{formatTime(session.timestamp)}</Text>
        ...
        <div className="flex-1 p-3">
          <div className="rounded-xl border border-kumo-line bg-kumo-control/50 p-3">
            <Text size="sm" bold>{t("sidebar_coming_soon_title")}</Text>
            <span className="mt-1 block">
              <Text size="xs" variant="secondary">{t("sidebar_coming_soon_desc")}</Text>
            </span>
          </div>
        </div>
      )}
