...
const BUILTIN_TOOL_LIST: string[] = [
  ...
  `${BUILTIN_CHART_TEMPLATE_KEY}: Get the exact format spec for a chart type. Only for complex types (sankey, treemap, candlestick, dualAxes, vega-lite, mermaid erDiagram/gitGraph). Common chart types — generate directly.`,
  `${BUILTIN_MATH_EVAL_KEY}: Evaluate complex mathematical expressions (3+ operations, sqrt/ln/sin, unit conversions, statistics). Simple arithmetic — answer directly.`,
