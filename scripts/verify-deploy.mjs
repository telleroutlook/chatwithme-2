...
async function verifyBaseUrl(baseUrl) {
  ...
  expect(/text\/html/i.test(indexContentType), `index content-type must be text/html, got: ${indexContentType || 'unknown'}`);
  ...
  for (const assetPath of assets) {
    ...
    if (assetPath.endsWith('.js')) {
      expect(looksLikeJavascript(contentType), `JS asset ${assetPath} returned unexpected content-type: ${contentType || 'unknown'}`);
    }

    if (assetPath.endsWith('.css')) {
      expect(/text\/css/i.test(contentType), `CSS asset ${assetPath} returned unexpected content-type: ${contentType || 'unknown'}`);
    }
  }

  console.log(`[verify] OK: ${assets.length} assets checked (${jsAssets.length} js, ${cssAssets.length} css)`);
}
...
async function main() {
  ...
  expect(urlSet.size > 0, 'no deployment URLs provided. Set DEPLOY_VERIFY_URLS or ensure wrangler.jsonc vars.HOST exists.');
